...
def exhaustive_mixed_candidates(length, alphabet):
    """Lazily yields every unique fixed-length string; repetition is allowed."""
    if length <= 0:
        return
    ...
    while True:
        ...
        while pos >= 0:
            indexes[pos] += 1
            if indexes[pos] < len(alphabet):
                break
            indexes[pos] = 0
            pos -= 1
        if pos < 0:
            return
# KOBY EXHAUSTIVE MIXED CHARACTER GENERATOR END
...
def readable_word_variants(word):
    """
    Generate every sensible number-substitution variant of an alphabetic word.
    Substitutions are made only where the resulting spelling still visibly
    resembles the source word. The original is included and duplicates vanish.
    """
    ...
    seen = set()
    eligible = [i for i, ch in enumerate(word) if ch in LEET_SUBSTITUTIONS]
    ...
    # Enumerate all substitution masks without a global artificial cap.
    # A mask replaces only eligible letters; the source word remains recoverable.
    for mask in range(1, 1 << len(eligible)):
        chars = list(word)
        substitutions = 0
        for bit, idx in enumerate(eligible):
            if mask & (1 << bit):
                chars[idx] = LEET_SUBSTITUTIONS[word[idx]][0]
                substitutions += 1

        candidate = "".join(chars)

        # Conservative readability checks:
        # - preserve at least one original alphabetic character
        # - never turn a word entirely into digits
        # - avoid consecutive digit runs of 3+ unless the source itself is short
        if not any(c.isalpha() for c in candidate):
            continue
        if len(candidate) >= 5 and re.search(r"\d{3,}", candidate):
            continue
        if substitutions > max(1, len(word) // 2):
            continue

        value = add(candidate)
        if value:
            yield value
# KOBY READABLE WORD VARIANTS END
